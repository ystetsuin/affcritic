...
Stats Collector: окремий Python скрипт (`stats_collector.py`) → збирає subscribers, avatar, bio, views, forwards, replies для кожного active каналу → `channel_stats_history` + `raw_posts` + `channels`. Окремий cron (раз на добу).
...
| БД | PostgreSQL (Neon/Supabase) + pgvector |
...
raw_posts (
  ...
  media_url TEXT,
  posted_at TIMESTAMP,
  embedding vector(1536),  -- pgvector, cosine similarity via <=> operator
...
Історія підписників каналу. Записується stats_collector.py при кожному запуску. Один запис per channel per run.
...
pipeline_logs (
  id UUID PRIMARY KEY,
  type TEXT NOT NULL CHECK (type IN ('scraper', 'embedding', 'grouping', 'gpt', 'quality', 'admin', 'stats')),
  post_id UUID REFERENCES posts(id),
  payload JSONB,
  created_at TIMESTAMP DEFAULT now()
)
...
CREATE INDEX idx_channel_stats_history ON channel_stats_history(channel_id, scraped_at);
CREATE INDEX idx_raw_posts_embedding ON raw_posts USING hnsw (embedding vector_cosine_ops);
```
...
| `/channels/` | Каталог каналів (card list з sparkline) |
| `/channels/{username}/` | Профіль каналу: hero + analytics + feed |
...
- Card list: avatar + name/bio + subscribers (дельта 7д) + posts + uniq% + sparkline (30д) + top tags
- Фільтр по категоріях (sidebar)
- Сортування: Підписники / Активність / Унікальність
- Клік на картку → `/channels/{username}/`

**Сторінка /channels/{username}/:**

- ChannelHero: avatar, name, bio, categories, TG link
- SubscriberStats: поточне число + дельти + area chart
- SubscriberGrowthChart: delta bar chart (Тиждень/Місяць/Весь час)
- ReachStats: ~median views/пост, ER%, period switcher (7д/30д/Все), гістограма розподілу views
- TopPosts: top 10 по views/forwards/replies, metric+period switcher, viral 🔥 badge
- ChannelStatsCard: metrics + heatmap + top tags
- Feed: пости каналу
...
**Канали:** таблиця зі stats (raw 24h/7d, feed, unique%, last post), summary header (5 metric tiles), sortable колонки, фільтр Всі/Активні/Неактивні. Bulk operations: чекбокси + select all + sticky toolbar (активувати/деактивувати/додати категорію/видалити). Inline category popover (чекбокси замість select). Inline edit display name (click → input, Enter/Escape). При додаванні: strip @, multi-select категорій. Visual indicators: dead channel badges (>30д red, >7д amber), unique% кольори.
...
**Теги:** CRUD категорій і тегів, approve/reject pending, merge (autocomplete замість select), bulk operations (чекбокси + toolbar: перемістити/об'єднати/видалити), пошук, сортування (новіші/А-Я), алфавітний фільтр, collapse/expand категорій
...
affcritic/
├── app/
│   ├── layout.tsx                         # Root layout: fonts, critical CSS, preconnect, Topbar, MobileHeader, BottomNav, ThemeProvider, AdminWrapper
...
│   ├── channels/
│   │   ├── page.tsx                       # Channels catalog: card list with sparklines
│   │   └── [username]/page.tsx           # Channel profile: hero + subscribers + stats + feed
│   ├── tags/
...
│   │   ├── channels/page.tsx             # Channels admin: stats table, bulk ops, inline edit, category popover
...
│       ├── channels/                      # CRUD, stats, bulk-status, bulk-delete, bulk-add-category
│       ├── channel-stats/                 # Per-channel stats: metrics, heatmap, topTags, subscribers, reach
│       ├── channel-top-posts/             # Top 10 posts by views/forwards/replies
│       ├── stats/run/                     # Manual stats collector trigger
│       ├── folders/                       # Channel categories CRUD + M2M channels + reorder
│       ├── tags/                          # CRUD, approve, reject, merge, aliases, bulk-move, bulk-merge, bulk-delete
...
│   ├── PostSources.tsx                    # Expandable sources list (render children: toggle + panel)
...
│   ├── ChannelsPage.tsx                   # Client: channels catalog card list (avatar, sparkline, subs, tags)
│   ├── Sparkline.tsx                      # Server: inline SVG mini-graph (polyline)
│   ├── ChannelHero.tsx                    # Server: channel profile header (avatar, name, bio, TG link)
│   ├── SubscriberStats.tsx                # Client: subscriber count + deltas + area chart
│   ├── SubscriberGrowthChart.tsx           # Client: delta bar chart with period switcher
│   ├── ReachStats.tsx                     # Client: reach stats (median views, ER, histogram, periods)
│   ├── TopPosts.tsx                       # Client: top 10 posts by views/forwards/replies
│   ├── TopicsPage.tsx                     # Client: topics hub (category + tag tiles)
│   ├── ChannelStatsCard.tsx                # Client: per-channel stats (orchestrates all channel analytics)
│   ├── PostingHeatmap.tsx                 # Client: 7×24 posting heatmap (day × hour)
│   ├── TopTagsBar.tsx                     # Client: horizontal bar chart of top tags
...
│   ├── openai.ts                          # Embedding (pgvector write via $executeRaw) + GPT + quality (pgvector AVG in SQL)
│   ├── dedup.ts                           # pgvector cosine similarity via <=> operator (SQL), JS fallback
│   ├── grouping.ts                        # Group logic (raw SQL for embedding queries)
│   ├── pipeline.ts                        # Pipeline orchestration (raw SQL for embedding null checks)
...
├── prisma/
│   ├── schema.prisma                      # 14 models, 2 enums (7 PipelineLogType values)
│   ├── seed.ts
│   └── prisma.config.ts
└── scraper/
    ├── main.py                            # Telethon scraper: raw_posts + blocked_posts check
    ├── stats_collector.py                 # Telethon: subscribers, avatar, bio, views/forwards/replies → channel_stats_history + channels + raw_posts
    ├── auth.py
    ├── cron_runner.sh                     # Scraper cron wrapper (self-throttle)
    ├── stats_cron.sh                      # Stats collector daily cron (lock file)
    └── requirements.txt
```
...
6. Embedding в `raw_posts` як `vector(1536)` (pgvector). Cosine similarity обчислюється в SQL через `<=>`. Поле `Unsupported` в Prisma — всі запити до embedding через `$queryRaw`/`$executeRaw`
...
26. Critical CSS: inline `<style>` в `<head>` для `.hidden`, `.lg:hidden`, `.lg:block`, `.lg:grid` (FOUC prevention до завантаження Tailwind)
